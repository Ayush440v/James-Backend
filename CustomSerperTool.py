...
class CustomSerperDevTool(BaseTool):
    ...
    description: str = (
        ...
        "Supports different search types: 'search' (default), 'news', 'images', 'maps'"
    )
    ...
    def _get_search_url(self, search_type: str) -> str:
        ...
        allowed_search_types = ["search", "news", "images", "maps"]
    ...
    def _process_image_results(self, image_results: list) -> list:
        """Process image search results."""
        processed_results = []
        for result in image_results[: self.n_results]:
            try:
                result_data = {
                    "title": result.get("title", ""),
                    "imageUrl": result.get("imageUrl", ""),
                ...
                # Optionally include copyright/credit if present
                if "copyright" in result:
                    result_data["copyright"] = result["copyright"]
                if "credit" in result:
                    result_data["credit"] = result["credit"]
                processed_results.append(result_data)
            except Exception as e:
                logger.warning(f"Skipping malformed image result: {result} ({e})")
                continue
        return processed_results

    def _process_maps_results(self, places_results: list) -> list:
        """Process maps search results (places)."""
        processed_results = []
        for result in places_results[: self.n_results]:
            try:
                result_data = {
                    "position": result.get("position"),
                    "title": result.get("title", ""),
                ...
                logger.warning(f"Skipping malformed maps result: {result} ({e})")
                continue
        return processed_results
    ...
    def _process_search_results(self, results: dict, search_type: str) -> dict:
        ...
        if search_type == "search":
            ...
            if "images" in results:
                formatted_results["images"] = self._process_image_results(results["images"])

        elif search_type == "maps":
            if "places" in results:
                formatted_results["places"] = self._process_maps_results(results["places"])
            if "ll" in results:
                formatted_results["ll"] = results["ll"]
